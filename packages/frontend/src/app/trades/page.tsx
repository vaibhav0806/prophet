...
function PnlCell({ pnl }: { pnl: number | null }) {
  if (pnl === null) {
    return <span className="text-[#3D4350] font-mono tabular-nums">&mdash;</span>
  }
...
export default function TradesPage() {
  ...
  return (
    <div className="p-5 lg:p-6 page-enter">
      <h1 className="text-lg font-bold text-white mb-5" style={{ fontFamily: 'var(--font-serif), Georgia, serif' }}>Trade History</h1>

      {isLoading && offset === 0 && <SkeletonTable />}

      {!isLoading && allTrades.length === 0 && (
        <div className="py-8 text-center">
          <div className="text-xs font-mono text-[#3D4350]">NO TRADES YET</div>
          <div className="text-xs text-[#1C2030] mt-1">Start the agent to begin executing arbitrage trades</div>
        </div>
      )}
                    ...
                    <Fragment key={trade.id}>
                      ...
                      >
                        ...
                        <td className="py-2.5 text-right font-mono text-xs text-[#6B7280]" title={closedRel?.full}>
                          {closedRel ? closedRel.relative : <span className="text-[#3D4350]">&mdash;</span>}
                        </td>
                      </tr>
